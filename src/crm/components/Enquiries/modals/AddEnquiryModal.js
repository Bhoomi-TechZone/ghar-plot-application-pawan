  ...
  const validateForm = () => {
    ...
    for (const field of required) {
      if (!formData[field] || (typeof formData[field] === 'string' && !formData[field].trim())) {
        CrossPlatformAlert.alert('Validation Error', `${field.replace(/([A-Z])/g, ' $1').trim()} is required`);
        return false;
      }
    }
    ...
    if (!phoneRegex.test(formData.contactNumber)) {
      CrossPlatformAlert.alert('Validation Error', 'Please enter a valid contact number');
      return false;
    }
    ...
    try {
      ...
      if (result && result.success) {
        ...
        if (formData.weekOrActionTaken && formData.weekOrActionTaken.trim()) {
          ...
          CrossPlatformAlert.alert('Success', result.message || 'Enquiry created successfully!', [
      ...
      CrossPlatformAlert.alert('Error', `Failed to create manual inquiry: ${error.message || 'Unknown error'}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const createReminderFromEnquiry = async (enquiryData) => {
    try {
      ...
      if (!reminderDateTime) {
        ...
        CrossPlatformAlert.alert('Warning', 'Invalid reminder date/time format. Reminder was not created.');
        return;
      }
      ...
      CrossPlatformAlert.alert('Error', `Failed to create reminder: ${error.message}`);
    }
  };
